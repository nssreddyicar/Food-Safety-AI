...
## Recent Changes
- **Admin Panel for FBO Inspections**: Full-featured admin panel at `/admin/fbo-inspections` with 4 tabs: Overview, Inspection Types, Deviation Categories, and Action Types. Features full CRUD for inspection types (7 defaults: Routine, Follow-up, Surveillance, Complaint-Based, Special Drive, License Verification, Pre-License), deviation categories (10 defaults with severity levels and FSSAI legal references), and action types (10 defaults from Verbal Warning to Emergency Prohibition with follow-up requirements). Includes **Reset to Defaults** buttons for all entities.
