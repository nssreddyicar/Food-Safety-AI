    ...
    <View style={[styles.container, { backgroundColor: theme.backgroundRoot }]}>
      ...
      >
        <View style={[styles.card, { backgroundColor: theme.backgroundDefault }, Shadows.md]}>
          ...
          <InfoRow icon="map-pin" label="Address" value={inspection.fboDetails.address} />
          <InfoRow icon="calendar" label="Date" value={formattedDate} />
          {inspection.fboDetails.licenseNumber ? (
            <InfoRow icon="file-text" label="License Number" value={inspection.fboDetails.licenseNumber} />
          ) : null}
        </View>

        <View style={[styles.card, { backgroundColor: theme.backgroundDefault }, Shadows.md]}>
          <ThemedText type="h3" style={styles.sectionTitle}>Proprietor Details</ThemedText>
          <InfoRow icon="user" label="Name" value={inspection.proprietorDetails.name} />
          <InfoRow icon="phone" label="Phone" value={inspection.proprietorDetails.phone} />
          {!inspection.proprietorDetails.isSameAsFBO ? (
            <InfoRow icon="home" label="Address" value={inspection.proprietorDetails.address} />
          ) : null}
        </View>
        ...
        {inspection.deviations.length > 0 ? (
          <View style={[styles.card, { backgroundColor: theme.backgroundDefault }, Shadows.md]}>
            ...
            {inspection.deviations.map((deviation, index) => (
              <View key={deviation.id} style={[styles.deviationItem, { borderLeftColor: theme.warning }]}>
                <View style={styles.deviationHeader}>
                  ...
                  <View style={[styles.severityBadge, { backgroundColor: theme.warning + '20' }]}>
                    <ThemedText type="small" style={{ color: theme.warning, textTransform: 'capitalize' }}>
                      {deviation.severity}
                    </ThemedText>
                  </View>
                </View>
        ...
        {inspection.actionsTaken.length > 0 ? (
          <View style={[styles.card, { backgroundColor: theme.backgroundDefault }, Shadows.md]}>
            <ThemedText type="h3" style={styles.sectionTitle}>Actions Taken</ThemedText>
            <View style={styles.actionsGrid}>
              {inspection.actionsTaken.map((action) => (
                <View key={action} style={[styles.actionTag, { backgroundColor: theme.success + '15' }]}>
                  <Feather name="check-circle" size={14} color={theme.success} />
                  <ThemedText type="small" style={{ color: theme.success }}>{action}</ThemedText>
                </View>
              ))}
            </View>
          </View>
