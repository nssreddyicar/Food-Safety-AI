...
import { View, StyleSheet, Pressable, FlatList, Modal, TouchableOpacity, ActivityIndicator, TextInput } from 'react-native';
...
export default function SampleCodeBankScreen() {
  ...
  const { data: codesData, isLoading, refetch } = useQuery<{ codes: SampleCode[]; counts: { available: number; used: number } }>({
    queryKey: ['/api/sample-codes', activeTab, statusFilter, jurisdictionId, searchQuery],
    queryFn: async () => {
  ...
  const { data: codeDetails } = useQuery<{ code: SampleCode; auditLog: AuditLogEntry[] }>({
  ...
  const renderCodeItem = useCallback(({ item }: { item: SampleCode }) => {
    ...
    return (
      ...
      >
        <View style={styles.codeContent}>
          <ThemedText type="h3" style={{ fontFamily: 'monospace', letterSpacing: 2 }}>
            {item.fullCode}
          </ThemedText>
          <View style={styles.codeDetails}>
            <View style={[
              styles.statusBadge,
              { backgroundColor: isUsed ? theme.accent + '15' : theme.success + '15' }
            ]}>
              <View style={[
                styles.statusDot,
                { backgroundColor: isUsed ? theme.accent : theme.success }
              ]} />
              <ThemedText type="small" style={{ color: isUsed ? theme.accent : theme.success, fontWeight: '600' }}>
  ...
  const renderStatusChip = (status: StatusFilter, label: string, count?: number) => {
      ...
      <View style={[styles.header, { paddingTop: headerHeight + Spacing.md }]}>
        ...
        <View style={styles.statsRow}>
          <View style={[styles.statCard, { backgroundColor: theme.success + '15' }]}>
            ...
            <ThemedText type="small" style={{ color: theme.success }}>Available</ThemedText>
          </View>
          <View style={[styles.statCard, { backgroundColor: theme.accent + '15' }]}>
            ...
            <ThemedText type="small" style={{ color: theme.accent }}>Used</ThemedText>
          </View>
        </View>

        <View style={styles.filtersRow}>
          {renderStatusChip('all', 'All')}
          {renderStatusChip('available', 'Available', codesData?.counts?.available)}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <ThemedText type="h3" style={{ color: theme.textSecondary, marginTop: Spacing.md }}>
                No Sample Codes
              </ThemedText>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: theme.backgroundDefault }]}>
            ...
            <View style={[styles.sampleTypeBadge, { backgroundColor: activeTab === 'enforcement' ? theme.accent + '15' : theme.success + '15' }]}>
              <ThemedText type="body" style={{ color: activeTab === 'enforcement' ? theme.accent : theme.success, fontWeight: '600' }}>
                {activeTab === 'enforcement' ? 'Enforcement Samples' : 'Surveillance Samples'}
              </ThemedText>
            </View>
            ...
            <View style={styles.quantityRow}>
              <ThemedText type="body" style={{ fontWeight: '600' }}>Quantity to Generate:</ThemedText>
              <TextInput
                value={quantity}
                onChangeText={setQuantity}
                keyboardType="numeric"
                style={[styles.quantityInput, { backgroundColor: theme.backgroundRoot, borderColor: theme.border, color: theme.text }]}
              />
            </View>

            <View style={styles.previewBox}>
              <ThemedText type="small" style={{ color: theme.textSecondary, marginBottom: Spacing.xs }}>Preview (first code):</ThemedText>
              <ThemedText type="h3" style={{ fontFamily: 'monospace', letterSpacing: 2 }}>
                {prefixStart.padStart(3, '0')}-{middleStart.padStart(3, '0')}-{suffixStart.padStart(3, '0')}
              </ThemedText>
            </View>
            ...
            >
              {generateMutation.isPending ? 'Generating...' : `Generate ${quantity} Codes`}
            </Button>
          </View>
        </View>
      </Modal>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: theme.backgroundDefault }]}>
            ...
            {codeDetails?.code ? (
              <>
                <View style={[styles.codeDisplay, { backgroundColor: theme.primary + '10' }]}>
                  <ThemedText type="h1" style={{ fontFamily: 'monospace', letterSpacing: 3, color: theme.primary }}>
                    {codeDetails.code.fullCode}
                  </ThemedText>
                  <View style={[
                    styles.statusBadge,
                    { backgroundColor: codeDetails.code.status === 'used' ? theme.accent + '15' : theme.success + '15', marginTop: Spacing.md }
                  ]}>
                    <View style={[
                      styles.statusDot,
                      { backgroundColor: codeDetails.code.status === 'used' ? theme.accent : theme.success }
                    ]} />
                    <ThemedText type="body" style={{ color: codeDetails.code.status === 'used' ? theme.accent : theme.success, fontWeight: '600' }}>
                ...
                <View style={styles.detailsSection}>
                  <View style={styles.detailRow}>
                    <ThemedText type="small" style={{ color: theme.textSecondary }}>Sample Type</ThemedText>
                    <ThemedText type="body" style={{ fontWeight: '600', textTransform: 'capitalize' }}>
                      {codeDetails.code.sampleType}
                    </ThemedText>
                  </View>
                  <View style={styles.detailRow}>
                    <ThemedText type="small" style={{ color: theme.textSecondary }}>Generated</ThemedText>
                    <ThemedText type="body">
                      {new Date(codeDetails.code.generatedAt).toLocaleString()}
                    </ThemedText>
                  </View>
                  {codeDetails.code.status === 'used' && codeDetails.code.usedAt ? (
                    <>
                      <View style={styles.detailRow}>
                        <ThemedText type="small" style={{ color: theme.textSecondary }}>Used At</ThemedText>
                        <ThemedText type="body">
                      ...
                      {codeDetails.code.linkedSampleReference ? (
                        <View style={styles.detailRow}>
                          <ThemedText type="small" style={{ color: theme.textSecondary }}>Linked Sample</ThemedText>
                ...
                {codeDetails.auditLog?.length > 0 ? (
                  <View style={styles.auditSection}>
                    <ThemedText type="h4" style={{ marginBottom: Spacing.md }}>Audit Trail</ThemedText>
                    {codeDetails.auditLog.map((entry) => (
                      <View key={entry.id} style={[styles.auditEntry, { borderColor: theme.border }]}>
                        <View style={[styles.auditDot, { backgroundColor: entry.action === 'used' ? theme.accent : theme.primary }]} />
