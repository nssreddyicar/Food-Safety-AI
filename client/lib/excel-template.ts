...
export function generateExcelCSV(data: ExcelReportData): string {
  ...
  lines.push(`Date Range,${formatDate(dateRange.startDate)} - ${formatDate(dateRange.endDate)}`);
  ...
  lines.push(`License Fees Collected,Rs ${metrics.licenses.amount.toLocaleString('en-IN')}`);
  lines.push('');
  ...
  lines.push(`Registration Fees Collected,Rs ${metrics.registrations.amount.toLocaleString('en-IN')}`);
  lines.push('');
  ...
  lines.push(`Total Inspections,${metrics.inspections.license + metrics.inspections.registration}`);
  lines.push('');
  ...
  lines.push(`Total FSW Activities,${metrics.fsw.testing + metrics.fsw.training + metrics.fsw.awareness}`);
  lines.push('');
  ...
  lines.push(`License Fees,${metrics.licenses.total} licenses,Rs ${metrics.licenses.amount.toLocaleString('en-IN')}`);
  lines.push(`Registration Fees,${metrics.registrations.total} registrations,Rs ${metrics.registrations.amount.toLocaleString('en-IN')}`);
  lines.push(`Total Revenue,${metrics.licenses.total + metrics.registrations.total} transactions,Rs ${(metrics.licenses.amount + metrics.registrations.amount).toLocaleString('en-IN')}`);
  lines.push('');
  lines.push('');
